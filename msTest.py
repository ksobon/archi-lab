...
rhDllPath = appDataPath + r"\Dynamo\0.8\packages\Mantis Shrimp\bin\RhinoCommon.dll"
if msPath not in sys.path:
	sys.path.Add(msPath)
...
class MSMesh(object):
        ...
        def toRHMesh(self):		
                ...
                for i in self.faceTopology:
                        if len(i) == 3:
                                rhMeshFace = rc.Geometry.MeshFace(i[0], i[1], i[2])
                        else:
                                rhMeshFace = rc.Geometry.MeshFace(i[0], i[1], i[2], i[3])
